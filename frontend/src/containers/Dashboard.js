...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    // Réinitialisation de l'état précédent si un autre index était ouvert
    if (this.index !== index) {
    ...
    if (this.openedTickets && this.openedTickets.length > 0) {
      this.openedTickets.forEach((ticketId) => {
        $(`#open-bill${ticketId}`).css({ background: "#0D5AE5" });
      });
    }

    if (this.counter % 2 === 1) {
      this.openedTickets = filteredBills(bills, getStatus(this.index)).map((bill) => bill.id);
    } else {
      this.openedTickets = [];
    }
